...
function getJSON(url) {
  ...
  // model code...it is a bit redundant in this case...we could just call getJSON directly...but if our model became 
  //more complex this sets us up to accomodate that.
  ...
  function renderShipList(ships, shipListElement) {
    ...
    ships.forEach(function (ship) {
      ...
      listItem.addEventListener("click", function (event) {
        //when clicked the default link behavior should be stopped, and the ship details function should be called...
        //passing the value of the href attribute in
  ...
  function showShips(url = "https://swapi.dev/api/starships/") {
    getShips(url).then(function (data) {
      ...
      if (data.next) {
        ...
        // normally we would prefer the addEventListener method of adding a listener. Using something like 
        //'element.onEvent = event_function' has the limitation of only being able to hold one listener of the 
        //type we choose. In this case that is a good thing however. Because we are not re-creating the buttons each 
        //time we load a new batch of data we could end up with several listeners attached to each button by the last 
        //page. We won't have that issue here.
        next.ontouchend = () => {
